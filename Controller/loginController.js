...
var instance = new Razorpay({
  key_id: "rzp_test_99ev8foDTy64Tk",
  key_secret: "btuL1bQteuZrjOOScnkIpD0x",
});
      ...
      if (resolve == null) {
        ...
        if (isNaN(userEmail)) {
          res.render("user/login", {
            mssgOne: "This email / mobile is not registered",
          });
        } else if (
          parseInt(userEmail) == "number" ||
          typeof parseInt(userEmail) == "number"
        ) {
          ...
          Users.findOne({ phone: phone })
            ...
            .catch((err) => {
              ...
              res.render("user/loginWithOtp", {
                alertOne: "please check your internet",
              });
            });
        }
      });
    } else {
      const user = resolve.username;
      phone = resolve.phone;
      if (resolve.userStatus == "block") {
        res.render("user/login", { mssgOne: "This user is blocked" });
      } else {
    ...
    .then(async (resolve) => {
      ...
      if (method == "COD") {
        ...
        if (oldAddress == "active") {
          ...
          orders.save().then(async (resolve) => {
            if (applyCoupon > 0) {
              ...
              userCouponModel
                ...
                .then(async (resolve) => {
                  const razorpay = await generateRazorpay(
                    resolve._id,
                    totalAmount.total - parseInt(ds_amount)
                  );
                  res.json(razorpay);
                });
            } else {
...
const orderPlaced = async (req, res) => {
    if(req.session.userloggedIn){
        const userId = req.session.user._id.valueOf();
        const orderId = req.query;
        const orderItem = await currentOrderItem(orderId);
      
        orderModel.findOne({ _id: ObjectId(orderId) }).then((resolve) => {
          const username = resolve.deliveryDetails.username;
          cartModel.deleteOne({ userId: userId }).then((resolve) => {
            res.render("user/orderplaced", {
              username,
              products: orderItem.items,
  ...
  orderModel
    ...
    .then((resolve) => {
      orderModel
        ...
        .then((resolve) => {
          res.render("user/orderdetails", { resolve, username: req.session.user });
        });
    });
    }
    else{
...
const trackOrder = (req, res) => {
  ...
  orderModel
    ...
    .then((resolve) => {
      console.log(resolve);
      res.render("user/trackOrder", { resolve, user: req.session.user });
    });
};
...
const searchItem = (req, res) => {
  const { text } = req.body;
  const user = req.session.userloggedIn ? req.session.user : null
  productModel
    .find({ $text: { $search: text } }, { score: { $meta: "textScore" } })
    .sort({ score: { $meta: "textScore" } })
    .then((resolve) => {
      res.render("user/searchItems", { resolve, text, user });
    })
    .catch((err) => {
      console.log(err);
    });
...
const getCoupon = (req, res) => {
  const userId = req.session.user._id;
  couponModel.find({ users: { $nin: userId } }).then((resolve) => {
    const coupon = resolve;
    userCouponModel.find({ userId: userId }).then((resolve) => {
...
const claimCoupon = (req, res) => {
  ...
  couponModel
    ...
    .then((resolve) => {
      const { couponName, amount, requiredAmount, image } = resolve;
...
const applyCoupon = (req, res) => {
  const userId = req.session.user._id;
  const { couponCod, totalAmount } = req.body;
  ...
  userCouponModel
    ...
    .then((resolve) => {
      if (!resolve) {
        ...
        const { requiredAmount, amount } = resolve;
...
const saveAddress = (req, res) => {
  console.log(req.body);
  const { username, phone, locality, district, city, state, pincode, email } =
    req.body;
  const userId = req.session.user._id;
